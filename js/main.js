...
createBtn.addEventListener("click", (event) => {
  ...
  if (firstname.value.trim() == "" || lastname.value.trim() == "" ||  phone.value.trim() == "" || loc.value.trim() == "" || email.value.trim() == "" || photo.value.trim() == "") {
    alert("Enter Data!!!");
  }
  else if (!email.value.trim().includes("@") || !email.value.trim().includes(".") || email.value.trim().length < 6) {
...
function addCard(user) {
  $trambon.insertAdjacentHTML(
    ...
    <div class="row mt-2 jumbotron">
        <div class="img col-md-4 col-xs-12 col-sm-6 col-lg-4" style="background-image: url('${user.photo}');"></div>
